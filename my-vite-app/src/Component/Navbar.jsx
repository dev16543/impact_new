    ...
    return (
        <nav className={`navbar fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
            isMobile 
                ...
                : `w-full px-6 py-4 ${scrolled ? 'py-3' : 'py-6'}`
        }`}>
            {!isMobile && (
            ...
            {isMobile && (
                <>
                    <a href="/" className="flex items-center">
                        <img src={OSALogo} alt="Mobile Logo" className="h-8 object-contain" />
                    </a>
                    <button
                        className={`flex flex-col justify-between w-8 h-6 bg-transparent border-none cursor-pointer z-[100] ${isMenuOpen ? 'active' : ''}`}
                        onClick={() => setIsMenuOpen(!isMenuOpen)}
                        aria-label="Toggle navigation menu"
                        aria-expanded={isMenuOpen}
                    >
                        <span className={`block w-full h-0.5 bg-white rounded-sm transition-all duration-300 drop-shadow-lg ${isMenuOpen ? 'transform rotate-45 translate-y-2.5' : ''}`}></span>
                        <span className={`block w-full h-0.5 bg-white rounded-sm transition-all duration-300 drop-shadow-lg ${isMenuOpen ? 'opacity-0' : ''}`}></span>
                        <span className={`block w-full h-0.5 bg-white rounded-sm transition-all duration-300 drop-shadow-lg ${isMenuOpen ? 'transform -rotate-45 -translate-y-2.5' : ''}`}></span>
                    </button>
